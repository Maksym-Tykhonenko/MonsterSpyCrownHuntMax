import MonsterSpyPagesProvider from './monserSpySrc/monsterSpyScreens/MonsterSpyPagesProvider';
import { NavigationContainer } from '@react-navigation/native';
import { createNativeStackNavigator } from '@react-navigation/native-stack';
import MonsterSpyOnboarding from './monserSpySrc/monsterSpyScreens/MonsterSpyOnboarding';
import { GestureHandlerRootView } from 'react-native-gesture-handler';
import { SafeAreaProvider } from 'react-native-safe-area-context';
import { MonsterSpyUserContext } from './monserSpySrc/monsterSpyContext/MonsterSpyUserContext';
import React, { useState, useEffect, useRef } from 'react';
import MonsterSpyLoading from './monserSpySrc/monsterSpyScreens/MonsterSpyLoading';
import MonMonsterSpyCrownHuntPrScr from './monserSpySrc/monsterSpyScreens/MonsterSpyCrownHuntPrScr';
const RootStackNav = createNativeStackNavigator();
const rootWrapperStyle = { flex: 1 };
// libs
...
const MonsterStack = () => {
  ...
  const INITIAL_URL = `https://free-unit-cell.space/`;
  const URL_IDENTIFAIRE = `EqhSmG8J`;
  ...
  OneSignal.initialize('1523f125-d0ea-408d-aea0-21ae6d922e54');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2025-08-20T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
      ...
      return (
        <RootStackNav.Navigator>
          <RootStackNav.Screen
            ...
            name="MonMonsterSpyCrownHuntPrScr"
            component={MonMonsterSpyCrownHuntPrScr}
    ...
    return (
      <RootStackNav.Navigator initialRouteName="MonsterSpyOnboarding">
        <RootStackNav.Screen
          name="MonsterSpyPagesProvider"
          component={MonsterSpyPagesProvider}
          options={{
            headerShown: false,
            animation: 'fade',
        ...
        />
        <RootStackNav.Screen
          name="MonsterSpyLoading"
          component={MonsterSpyLoading}
          options={{ headerShown: false }}
        />
        <RootStackNav.Screen
          name="MonsterSpyOnboarding"
          component={MonsterSpyOnboarding}
          options={{ headerShown: false }}
        />
      </RootStackNav.Navigator>
    );
  ...
  return (
    <NavigationContainer>
      {!loader ? <MonsterSpyLoading /> : <Route isFatch={route} />}
    </NavigationContainer>
  );
};

const MonsterSpyNavigation = () => {
  return (
    <GestureHandlerRootView style={rootWrapperStyle}>
      <MonsterSpyUserContext>
        <SafeAreaProvider>
          <MonsterStack />
        </SafeAreaProvider>
      </MonsterSpyUserContext>
...
export default MonsterSpyNavigation;
